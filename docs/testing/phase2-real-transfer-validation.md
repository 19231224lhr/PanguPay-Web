...
在不改变交易协议的前提下，当前页面已补齐：

- 收款地址的本地二维码；
- 普通、快速和跨链模式对应的真实节点路径；
- 活动记录的完整 TXID、时间和已知阶段；
- FastEvidence、AssignAck/责任收据与 CFAA 的凭证链；
- ExposureShares 的精确金额比例；
- 担保组织的质押、节点与入口详情。

后端契约门禁现在直接读取 `gateway/server.go`，核对 20 个实际使用的 HTTP method + path，
不再只比较客户端字符串。2026-07-28 的本地联测确认前端和 Gateway 均返回 200、Gateway
健康为 `ok`，可查询 1 个真实担保组织及其质押和 Assign 入口信息。
